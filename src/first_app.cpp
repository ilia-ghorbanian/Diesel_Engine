...
namespace dsl {
    ...
    void FirstApp::EventHandlerTest(DslWindow *window){
        SDL_Event event;
        while (SDL_PollEvent(&event))
        {
            switch (event.type){
                case SDL_EventType::SDL_WINDOWEVENT:
                    if (event.window.event == SDL_WINDOWEVENT_RESIZED){
                        ...
                        window->doResized(window->getWindow(), width, height);
                    };
            }
        }
    }
    ...
    void FirstApp::recordComandbuffer(int imgIndex){
            ...
            renderPassInfo.framebuffer = dslSwapChain->getFrameBuffer(imgIndex);
            ...
            vkCmdBeginRenderPass(commandBuffers[imgIndex], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

            dslPipeline->bind(commandBuffers[imgIndex]);
            dslModel->bind(commandBuffers[imgIndex]);
            dslModel->draw(commandBuffers[imgIndex]);
    ...
    void FirstApp::drawFrame(DslWindow *dslWindow){

        uint32_t imageIndex;
        auto result = dslSwapChain->acquireNextImage(&imageIndex);
        if (result == VK_ERROR_OUT_OF_DATE_KHR){
                EventHandlerTest(dslWindow);
                recreateSwapchain();
                return;
            }
